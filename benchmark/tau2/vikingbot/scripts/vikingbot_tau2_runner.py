...
_TAU2_CURRENT_TIME_RE = re.compile(
    r"[Tt]he current time is (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?: \w+)?)"
)
...
def _format_tau2_time(tau2_time: str) -> str:
    """Reformat tau2 policy time to match vikingbot's native format.

    Input:  "2024-05-15 15:00:00 EST"
    Output: "2024-05-15 15:00 (Wednesday) (EST)"
    """
    from datetime import datetime

    parts = tau2_time.strip().split()
    tz = parts[2] if len(parts) >= 3 else "UTC"
    try:
        dt = datetime.strptime(f"{parts[0]} {parts[1]}", "%Y-%m-%d %H:%M:%S")
    except ValueError:
        return tau2_time
    return f"{dt.strftime('%Y-%m-%d %H:%M (%A)')} ({tz})"


def _patch_current_time(messages: list[dict], tau2_time: str | None) -> None:
    """Replace the injected system time with the tau2 dataset time in-place."""
    if not tau2_time:
        return
    formatted = _format_tau2_time(tau2_time)
    replacement = f"{CURRENT_TIME_HEADER} {formatted}"
    for msg in messages:
        content = msg.get("content")
        if not isinstance(content, str) or CURRENT_TIME_HEADER not in content:
            continue
        patched = re.sub(
            rf"{re.escape(CURRENT_TIME_HEADER)}[^\n]*",
            replacement,
            content,
            count=1,
        )
        if patched != content:
            msg["content"] = patched
            print(f"[Runner] Patched current time → {formatted}")
            return
...
async def _run_agent(
    ...
    tau2_current_time: str | None = None,
    ...
    _patch_current_time(messages, tau2_current_time)
    if system_prompt:
...
def main() -> None:
    ...
    tau2_time_match = _TAU2_CURRENT_TIME_RE.search(policy or "")
    tau2_current_time = tau2_time_match.group(1) if tau2_time_match else None
    if tau2_current_time:
        print(f"[Runner] tau2 current time from policy: {tau2_current_time}")
    else:
        print("[Runner] No current time found in policy; system time will be used.")
    ...
    final_content, final_reasoning_content, tools_used, token_usage, iteration, memory_content = (
        asyncio.run(
            _run_agent(
                ...
                tau2_current_time=tau2_current_time,
            )
        )
    )
